...
def main_simulation(args):
    ...
    #### Entry rates from uniform
    entry_rates[entered_res] = np.random.random(size=len(entered_res))*10 #use 10 for the paper as in CR Chemostat.
    #### Constant entry rates
##    entry_rates[entered_res] = entry_rate_mean #constant entry rates across simulations.

    #### Entry rates for CR-batch-like simulation
##    entry_rates = np.random.random(size=3)*10
##    entry_rates[2] = 0
    print('entry_rates',
          entry_rates)
    ...
    if set_common_rand_seed==True:
        np.random.seed(sim) #this way initial condition across replicates are random,
                          #but the same every time we run the simulations.
    species_abus = np.random.random(N_species)
    resource_abus = np.random.random(N_resources)
    if set_common_rand_seed==True:
        #Reset to random seed for the simulation itself such that dynamics are unpredictable.
        np.random.seed()
    ...
    sigma_outside = 0.001 #use 1 for the simulations as in the paper CR chemostat.
    sigma_inside = 0.001
    thres = 1e-10
    ...
    for step in range(nsteps):   
        ...
        species_abus += d_sp*dt+np.random.normal(0,0.1,size=(N_species))*np.sqrt(dt)
        ''' Get colored noise for resource abundances, with autocorrelation time tau '''  
...
##            +np.sqrt(1-np.exp(-2*dt/tau))*np.random.normal(0,sigma_inside,size=(N_resources)))*np.sqrt(dt)*(entry_rates<thres) #noise for cross-fed resources is small
        ...
        if dataset_name == 'two_resources_one_high_one_low':
            outside_resource_noise[1] /= np.sqrt(2) #reduce the variance in the second resource by half.
...
dataset_name = '2025.12.19.100_species_5_fed_resources'
...
growth_choices = np.copy(growth_choices_complete)

if __name__=='__main__':

    how_many_simulations = 500
    nsteps=20000
    ...
    entered_res = np.random.choice(np.arange(1,min(independent_species_cutoff,N_resources)),
                                   size=how_many_supplied_resources-1, #choose one less since we will always supply resource 0
                                   replace=False); entry_rates[0] = 1

    
    entry_rates[entered_res] = 1
    supplied_resources = np.arange(min(independent_species_cutoff,N_resources))[entry_rates[:min(independent_species_cutoff,N_resources)]>0]
    with open('supplied_resources.{}.dat'.format(dataset_name),'wb') as f:
        pickle.dump(supplied_resources,f)
        f.close()
    print('Supplied Resources across all simulations')
